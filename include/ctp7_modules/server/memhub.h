...
/*!
 *  \brief This library is a thin wrapper around libmemsvc, which adds semaphores to synchronize concurrent
 *         read/write operations from different processes.
 */
int memhub_open(memsvc_handle_t *handle);
int memhub_close(memsvc_handle_t *handle);

/*!
 *  \brief These functions return -1 on error and 0 on success.
